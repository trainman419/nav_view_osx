...
MAP* load_map()
{
  ...
  map_frame = resp.map.header.frame_id;
...
void robot_footprint_callback(const geometry_msgs::PolygonStamped::ConstPtr& msg)
{
  ...
  in.header = out.header = msg->header;
  if( map_frame.length() > 0 ) {
    out.header.frame_id = map_frame;
  }

  if(!tf_listener->waitForTransform(out.header.frame_id, in.header.frame_id, 
  ...
  {
    in.point.x = msg->polygon.points[i].x;
    in.point.y = msg->polygon.points[i].y;
    in.point.z = msg->polygon.points[i].z;
    try {
      tf_listener->transformPoint(out.header.frame_id, in, out);
    } catch(tf::TransformException & e) {
      ROS_WARN("Transform Exception: %s", e.what());
      break;
    }
    robot->bound->points[i][0] = (out.point.x-map_x_offset)*x_scl;
    robot->bound->points[i][1] = (out.point.y-map_y_offset)*y_scl;
    robot->bound->points[i][2] = out.point.z*z_scl; 
  }
...
{     
  ...
  {
    in.point.x = msg->poses[i].pose.position.x;
    in.point.y = msg->poses[i].pose.position.y;
    in.point.z = msg->poses[i].pose.position.z;
    try {
      tf_listener->transformPoint(out.header.frame_id, in, out);
    } catch(tf::TransformException & e) {
      ROS_WARN("Transform Exception: %s", e.what());
    ...
    global_path->points[i][0] = x_scl* (out.point.x-map_x_offset);
    global_path->points[i][1] = y_scl* (out.point.y-map_y_offset);
    global_path->points[i][2] = z_scl*  out.point.z;   
  }
...
{     
  ...
  for(int i = 0; i < length; i++)
  {
    ...
    local_path->points[i][0] = x_scl*(out.point.x-map_x_offset);
    local_path->points[i][1] = y_scl*(out.point.y-map_y_offset);
    local_path->points[i][2] = z_scl* out.point.z;   
  }
...
{     
  ...
  for(int i = 0; i < length; i++)
  {
    in.point.x = msg->cells[i].x;
    in.point.y = msg->cells[i].y;
    in.point.z = msg->cells[i].z;
    try {
      tf_listener->transformPoint(out.header.frame_id, in, out);
    } catch(tf::TransformException & e) {
      ROS_WARN("Transform Exception: %s", e.what());
    ...
    inflated_obs_local->grids->points[i][0] = x_scl*(out.point.x-map_x_offset);
    inflated_obs_local->grids->points[i][1] = y_scl*(out.point.y-map_y_offset);
    inflated_obs_local->grids->points[i][2] = z_scl* out.point.z;   
  }
...
{      
 if(costmap == NULL)
  ...
  for(int i = 0; i < length; i++)
  {
    ...
    obstacles_local->grids->points[i][0] = x_scl*(out.point.x-map_x_offset);
    obstacles_local->grids->points[i][1] = y_scl*(out.point.y-map_y_offset);
    obstacles_local->grids->points[i][2] = z_scl*out.point.z;   
  }
...
void mouse(int button, int mouse_state, int x, int y)
{
  ...
  else if(display_state == SETPOSE)
  {
    ...
    else if(button == GLUT_LEFT_BUTTON && mouse_state == GLUT_UP)
    {
      ...
      msg.pose.pose.position.x = absolute_goal_x+map_x_offset;
      msg.pose.pose.position.y = absolute_goal_y+map_y_offset;
      ...
      if( r == 0)          
        ...
        msg.pose.pose.orientation.w = sin(theta)/r; 
...
{   
  ...
  robot_footprint_sub = nh.subscribe("/move_base/local_costmap/robot_footprint", 2, robot_footprint_callback);
  global_path_sub = nh.subscribe("/move_base/TrajectoryPlannerROS/global_plan", 2, global_plan_callback);
  ...
  new_pose_pub = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/initialpose", 2);

  tf_listener = new tf::TransformListener(nh);
  //tf_listener->setExtrapolationLimit(ros::Duration(1, 0));
